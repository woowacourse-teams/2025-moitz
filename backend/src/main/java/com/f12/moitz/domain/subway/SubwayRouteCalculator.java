    ...
    private SubwayRouteSearchResult searchShortestTimePath(
        ...
        while (searchState.hasNext()) {
            ...
            for (Edge edge : currentEdges) {
                ...
                final int movementTime = searchState.calculateTimeTo(currentStation, edge);
                final Optional<Integer> newTime = calculateTimeWithTransferIfNeeded(
                        searchState,
                        start,
                        end,
                        currentStation,
                ...
                searchState.recordIfShorter(currentStation, neighbor, currentLine, newTime.get());
            }
        }
        return searchState.toResult();
    }

    private Optional<Integer> calculateTimeWithTransferIfNeeded(
            final SubwayRouteSearchState searchState,
            final SubwayStation start,
            final SubwayStation end,
            final SubwayStation currentStation,
            final SubwayStation nextStation,
            final SubwayLine currentLine,
            final Set<Edge> currentEdges,
            final int movementTime
    ) {
        if (start.equals(currentStation) || searchState.canContinueOn(currentStation, currentLine)) {
            return Optional.of(movementTime);
        }

        final Optional<Edge> transferEdge = currentEdges.stream()
        ...
        return Optional.of(movementTime + transferEdge.get().getTimeInSeconds());
    }
